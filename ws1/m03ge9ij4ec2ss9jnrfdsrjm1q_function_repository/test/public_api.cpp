...
int main() {
    return test::run([] {
        ...
        test::expect(first_entry.call == &first_call);
        test::expect_equal(first_entry.ir.function_id, first_id);
        test::expect_equal(first_entry.ir.left, 10);
        test::expect_equal(first_entry.ir.right, 20);
        test::expect_equal(first_entry.ir.top, 30);
        test::expect_equal(first_entry.ir.bottom, 40);
        test::expect_equal(first_entry.ir.children.size(), std::size_t(1));
        test::expect_equal(first_entry.ir.connections.size(), std::size_t(1));
        ...
        test::expect_equal(g_first_call_count, 1);
        ...
        test::expect_equal(unchanged_entry.ir.left, 10);
        test::expect(unchanged_entry.call == &first_call);
        ...
        test::expect_equal(duplicate_entry.ir.left, 10);
        test::expect(duplicate_entry.call == &first_call);

        repository.save(make_ir(second_id, 50), &second_call);
        const auto second_entry = repository.load(second_id);
        test::expect(second_entry.call == &second_call);
        test::expect_equal(second_entry.ir.function_id, second_id);
        test::expect_equal(second_entry.ir.left, 50);
        g_second_call_count = 0;
        second_entry.call(function, 9);
        test::expect_equal(g_second_call_count, 1);
        test::expect_equal(g_last_argument, std::uint8_t(9));

        const api::function_repository_t::entry_t aggregate_entry {
            .call = &first_call,
            .ir = first_ir
        };
        test::expect(aggregate_entry.call == &first_call);
        test::expect_equal(aggregate_entry.ir.function_id, first_id);
        ...
        test::expect(empty_entry.call == &second_call);
        test::expect(!static_cast<bool>(empty_entry.ir.function_id));
        test::expect_equal(empty_entry.ir.left, 1);
    });
}
