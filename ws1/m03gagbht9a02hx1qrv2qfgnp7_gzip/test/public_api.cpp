...
int main() {
    return test::run([] {
        ...
        test::expect_equal(api::gzip(input, compressed), compressed);
        test::expect(filesystem_api::is_regular_file(input));
        test::expect_equal(read_file(input.to_native_path()), contents);
        test::expect(filesystem_api::is_regular_file(compressed));
        test::expect(!filesystem_api::exists(compressed + ".input"));
        test::expect(!filesystem_api::exists(compressed + ".input.gz"));

        test::expect_equal(api::ungzip(compressed, restored), restored);
        test::expect(filesystem_api::is_regular_file(compressed));
        test::expect_equal(read_file(restored.to_native_path()), contents);
        test::expect(!filesystem_api::exists(restored + ".input.gz"));
        test::expect(!filesystem_api::exists(restored + ".input"));
        ...
        test::expect(filesystem_api::is_regular_file(collision_output + ".input"));
        test::expect(!filesystem_api::exists(collision_output));
        ...
        test::expect(filesystem_api::is_regular_file(ungzip_collision + ".input.gz"));
        ...
        test::expect(!filesystem_api::exists(corrupt_output));
        test::expect(!filesystem_api::exists(corrupt_output + ".input.gz"));
        test::expect(!filesystem_api::exists(corrupt_output + ".input"));
    });
}
