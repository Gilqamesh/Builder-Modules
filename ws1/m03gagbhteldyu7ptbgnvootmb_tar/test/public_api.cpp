...
int main() {
    return test::run([] {
        ...
        test::expect_equal(api::tar(source, archive), archive);
        test::expect(filesystem_api::is_regular_file(archive));

        const filesystem_api::path_t extracted(
            temporary_directory.path() / "extracted"
        );
        test::expect_equal(api::untar(archive, extracted), extracted);
        test::expect_equal(
            read_file(extracted.to_native_path() / "root.txt"),
            std::string("root contents\n")
        );
        test::expect_equal(
            read_file(extracted.to_native_path() / "nested" / "value.txt"),
        ...
        test::expect_equal(api::untar(archive, existing_install), existing_install);
        test::expect_equal(
            read_file(existing_install.to_native_path() / "preexisting.txt"),
            std::string("keep\n")
        );
        ...
        test::expect_equal(api::tar(source, inside_archive), inside_archive);
        ...
        test::expect(filesystem_api::is_directory(existing_corrupt_output));
    });
}
