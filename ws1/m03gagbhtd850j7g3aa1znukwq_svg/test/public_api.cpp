...
int main() {
    return test::run([] {
        ...
        test::expect_equal(api::render_png(svg_file, output), output);
        test::expect(filesystem_api::is_regular_file(output));
        test::expect_equal(
            read_signature(output.to_native_path()),
            std::array<unsigned char, 8> { 0x89, 'P', 'N', 'G', 0x0d, 0x0a, 0x1a, 0x0a }
        );
        ...
        test::expect(!filesystem_api::exists(malformed_output));
    });
}
