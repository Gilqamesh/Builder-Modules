...
int main() {
    return test::run([] {
        ...
        test::expect(filesystem_api::is_directory(created));
        test::expect_equal(created.parent(), filesystem_api::path_t(workspace));

        const graph_api::module_name_t module_name(created.filename());
        test::expect_equal(module_name.friendly_name(), std::string("Example_Module9"));
        test::expect_equal(created.filename(), module_name.unique_name());
        ...
        test::expect(filesystem_api::is_regular_file(api_header));
        test::expect(filesystem_api::is_regular_file(cli_source));
        test::expect(filesystem_api::is_regular_file(builder_source));

        const auto header_contents = read_file(api_header.to_native_path());
        const auto expected_guard = uppercase(module_name.unique_name()) + "_API_H";
        test::expect(header_contents.find("#ifndef " + expected_guard) != std::string::npos);
        test::expect(header_contents.find("# define " + expected_guard) != std::string::npos);
        test::expect(
            header_contents.find("namespace " + module_name.unique_name())
                != std::string::npos
        );
        test::expect(
            header_contents.find("#endif // " + expected_guard)
                != std::string::npos
        );

        const auto cli_contents = read_file(cli_source.to_native_path());
        test::expect(cli_contents.find("#include \"api.h\"") != std::string::npos);
        test::expect(
            cli_contents.find("Hello from " + module_name.unique_name() + "!")
                != std::string::npos
        );
        test::expect(read_file(builder_source.to_native_path()).empty());

        const auto second = api::create("ws5", "Example_Module9");
        test::expect(filesystem_api::is_directory(second));
        test::expect_not_equal(second, created);
        test::expect_equal(
            graph_api::module_name_t(second.filename()).friendly_name(),
