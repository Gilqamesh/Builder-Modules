...
int main() {
    return test::run([] {
        ...
        test::expect_equal(api::write_dot(graph, *owner, dot_output), dot_output);
        test::expect(filesystem_api::is_regular_file(dot_output));

        const auto dot = read_file(dot_output.to_native_path());
        test::expect(dot.starts_with("digraph BuilderModuleGraph {\n"));
        test::expect(dot.find("subgraph cluster_workspace_0") != std::string::npos);
        test::expect(dot.find("subgraph cluster_workspace_1") != std::string::npos);
        test::expect(dot.find("label=\"ws0\"") != std::string::npos);
        test::expect(dot.find("label=\"ws1\"") != std::string::npos);
        test::expect(
            dot.find(std::format("builder\\n{}", owner_name.unique_name()))
                != std::string::npos
        );
        test::expect(
            dot.find(std::format("module\\n{}", owner_name.unique_name()))
                != std::string::npos
        );
        test::expect(
            dot.find(std::format(
        ...
        test::expect(
            dot.find("penwidth=2.2, color=\"#111111\"") != std::string::npos
        );
        ...
        test::expect_equal(api::render_svg(graph, *owner, svg_output), svg_output);
        test::expect(filesystem_api::is_regular_file(svg_output));
        test::expect(!filesystem_api::exists(temporary_dot));
        const auto svg = read_file(svg_output.to_native_path());
        test::expect(svg.find("<svg") != std::string::npos);
        test::expect(svg.find(owner_name.unique_name()) != std::string::npos);
