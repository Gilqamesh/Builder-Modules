...
extern "C" void phase__source(const m03gagbhsujjf63n0w3r2w4q6h_build_phases::source_phase_t* phase) {
    ...
    const auto google_test_tar_gz = m03gagbht7wqhtdg9hwdpmfn5o_download::fetch(
    ...
    const auto google_test_tar = m03gagbht9a02hx1qrv2qfgnp7_gzip::ungzip(
        google_test_tar_gz,
        phase->build_dir() / m03gagbhsnusi43zogoacgj2ez_filesystem::relative_path_t("google_test-1.15.0.tar")
    );
    const auto google_test_extract_dir = m03gagbhteldyu7ptbgnvootmb_tar::untar(
        google_test_tar,
