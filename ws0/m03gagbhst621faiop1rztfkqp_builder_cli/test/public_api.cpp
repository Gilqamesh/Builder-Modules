...
void expect_cli_output(
    ...
    test::expect_equal(lines.size(), arguments.size() + 2);
    test::expect_equal(lines[0], std::to_string(arguments.size() + 1));
    test::expect(!lines[1].empty());
    for (std::size_t i = 0; i < arguments.size(); ++i) {
        test::expect_equal(lines[i + 2], arguments[i]);
    }
}
