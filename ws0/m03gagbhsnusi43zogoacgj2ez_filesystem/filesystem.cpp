#include "filesystem.h"

#include <format>
#include <algorithm>
#include <iostream>
#include <fstream>

namespace m03gagbhsnusi43zogoacgj2ez_filesystem {

static std::filesystem::path append_postfix(const std::filesystem::path& path, std::string_view postfix) {
    if (postfix.find_first_of("/\\") != std::string_view::npos) {
        throw std::runtime_error(std::format("append_postfix: postfix '{}' contains path separator", postfix));
    }

    std::filesystem::path new_path = path;
    new_path.replace_filename(new_path.filename().string() + std::string(postfix));
    return new_path;
}

relative_path_t::relative_path_t(const std::filesystem::path& relative_path):
    m_relative_path(relative_path.lexically_normal())
{
    if (m_relative_path.is_absolute()) {
        throw std::runtime_error(std::format("relative_path_t::relative_path_t: path '{}' is absolute", m_relative_path.string()));
    }
}

const char* relative_path_t::c_str() const {
    return m_relative_path.c_str();
}

std::string relative_path_t::string() const {
    return m_relative_path.string();
}

std::string relative_path_t::stem() const {
    return m_relative_path.stem().string();
}

std::string relative_path_t::extension() const {
    return m_relative_path.extension().string();
}

const relative_path_t& relative_path_t::extension(std::string_view new_extension) {
    m_relative_path.replace_extension(new_extension);
    return *this;
}

bool relative_path_t::operator==(const relative_path_t& other) const {
    return m_relative_path == other.m_relative_path;
}

relative_path_t relative_path_t::operator/(const relative_path_t& other) const {
    relative_path_t result(to_native_path() / other.to_native_path());

    return result;
}

relative_path_t relative_path_t::operator+(std::string_view postfix) const {
    relative_path_t result(append_postfix(m_relative_path, postfix));

    return result;
}

const std::filesystem::path& relative_path_t::to_native_path() const {
    return m_relative_path;
}

path_t::path_t(const std::filesystem::path& path):
    m_path(std::filesystem::absolute(path).lexically_normal())
{
}

path_t path_t::parent() const {
    const auto parent_path = m_path.parent_path();
    if (parent_path == m_path) {
        throw std::runtime_error(std::format("path_t::parent: '{}' is a root path and has no parent", *this));
    }

    return path_t(parent_path);
}

bool path_t::is_child(const path_t& other) const {
    const auto rel = other.m_path.lexically_relative(m_path);
    return !rel.empty() && rel != "." && !rel.native().starts_with("..");
}

relative_path_t path_t::relative(const path_t& other) const {
    if (!is_child(other)) {
        throw std::runtime_error(std::format("path_t::relative: path '{}' is not a child of base path '{}'", other, *this));
    }

    return relative_path_t(other.m_path.lexically_relative(m_path));
}

bool path_t::is_sibling(const path_t& sibling) const {
    return m_path.parent_path() == sibling.m_path.parent_path();
}

std::string path_t::filename() const {
    return m_path.filename().string();
}

const char* path_t::c_str() const {
    return m_path.c_str();
}

std::string path_t::string() const {
    return m_path.string();
}

std::string path_t::stem() const {
    return m_path.stem().string();
}

std::string path_t::extension() const {
    return m_path.extension().string();
}

const path_t& path_t::extension(std::string_view new_extension) {
    m_path.replace_extension(new_extension);
    return *this;
}

bool path_t::operator==(const path_t& other) const {
    return m_path == other.m_path;
}

path_t path_t::operator/(const relative_path_t& relative_path) const {
    path_t result(m_path / relative_path.to_native_path());

    const auto rel = result.m_path.lexically_relative(m_path);
    if (rel.empty() || rel == "." || rel.native().starts_with("..")) {
        throw std::runtime_error(std::format("path_t::operator/: path '{}' must not escape the base path '{}'", result, *this));
    }

    return result;
}

path_t path_t::operator+(std::string_view postfix) const {
    path_t result(append_postfix(m_path, postfix));

    if (!result.is_sibling(*this) || result == *this) {
        throw std::runtime_error(std::format("path_t::operator+: path '{}' must be a strict sibling of base path '{}'", result, *this));
    }

    return result;
}

const std::filesystem::path& path_t::to_native_path() const {
    return m_path;
}

rooted_path_t::rooted_path_t(const path_t& root, const relative_path_t& relative_path):
    m_root(root),
    m_relative_path(relative_path)
{
    const auto selected_path = path();
    if (!exists(selected_path)) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::rooted_path_t::rooted_path_t: root '{}' has no entry at relative path '{}'", root, relative_path));
    }
}

const path_t& rooted_path_t::root() const {
    return m_root;
}

const relative_path_t& rooted_path_t::relative_path() const {
    return m_relative_path;
}

path_t rooted_path_t::path() const {
    return m_root / m_relative_path;
}

pretty_path_t::pretty_path_t(const path_t& path):
    m_string([&]() {
        const auto cwd = current_path();
        if (cwd.is_child(path)) {
            return cwd.relative(path).string();
        }
        return path.string();
    }())
{
}

const std::string& pretty_path_t::string() const {
    return m_string;
}

const char* pretty_path_t::c_str() const {
    return m_string.c_str();
}

find_include_predicate_t::find_include_predicate_t(std::function<bool(const path_t& path)>&& predicate):
    predicate(std::move(predicate))
{
}

bool find_include_predicate_t::operator()(const path_t& path) const {
    return predicate(path);
}

find_include_predicate_t find_include_predicate_t::operator&&(find_include_predicate_t b) const {
    return find_include_predicate_t {
        [=, this](const path_t& path) {
            return operator()(path) && b(path);
        }
    };
}

find_include_predicate_t find_include_predicate_t::operator||(find_include_predicate_t b) const {
    return find_include_predicate_t {
        [=, this](const path_t& path) {
            return operator()(path) || b(path);
        }
    };
}

find_include_predicate_t find_include_predicate_t::operator!() const {
    return find_include_predicate_t {
        [=, this](const path_t& path) {
            return !operator()(path);
        }
    };
}

find_descend_predicate_t::find_descend_predicate_t(std::function<bool(const path_t& dir, size_t depth)>&& predicate):
    predicate(std::move(predicate))
{
}

bool find_descend_predicate_t::operator()(const path_t& dir, size_t depth) const {
    return predicate(dir, depth);
}

find_descend_predicate_t find_descend_predicate_t::operator&&(find_descend_predicate_t b) const {
    return find_descend_predicate_t {
        [=, this](const path_t& dir, size_t depth) {
            return operator()(dir, depth) && b(dir, depth);
        }
    };
}

find_descend_predicate_t find_descend_predicate_t::operator||(find_descend_predicate_t b) const {
    return find_descend_predicate_t {
        [=, this](const path_t& dir, size_t depth) {
            return operator()(dir, depth) || b(dir, depth);
        }
    };
}

find_descend_predicate_t find_descend_predicate_t::operator!() const {
    return find_descend_predicate_t {
        [=, this](const path_t& dir, size_t depth) {
            return !operator()(dir, depth);
        }
    };
}

find_include_predicate_t find_include_predicate_t::include_all = {
    [](const path_t& path) {
        return true;
    }
};

find_include_predicate_t find_include_predicate_t::is_dir = {
    [](const path_t& path) {
        return is_directory(path);
    }
};

find_include_predicate_t find_include_predicate_t::is_regular = {
    [](const path_t& path) {
        return is_regular_file(path);
    }
};

find_include_predicate_t find_include_predicate_t::cpp_file = {
    [](const path_t& path) {
        return is_regular_file(path) && path.extension() == ".cpp";
    }
};

find_include_predicate_t find_include_predicate_t::c_file = {
    [](const path_t& path) {
        return is_regular_file(path) && path.extension() == ".c";
    }
};

find_include_predicate_t find_include_predicate_t::hpp_file = {
    [](const path_t& path) {
        return is_regular_file(path) && path.extension() == ".hpp";
    }
};

find_include_predicate_t find_include_predicate_t::h_file = {
    [](const path_t& path) {
        return is_regular_file(path) && path.extension() == ".h";
    }
};

find_include_predicate_t find_include_predicate_t::filename(const std::string& name) {
    return {
        [=](const path_t& path) {
            return path.filename() == name;
        }
    };
}

find_include_predicate_t find_include_predicate_t::path(const path_t& target) {
    return {
        [=](const path_t& path) {
            return path == target;
        }
    };
}

find_descend_predicate_t find_descend_predicate_t::descend_all = {
    [](const path_t& dir, size_t depth) {
        return true;
    }
};

find_descend_predicate_t find_descend_predicate_t::descend_none = {
    [](const path_t& dir, size_t depth) {
        return false;
    }
};

static std::vector<rooted_path_t> find_impl(
    const path_t& root,
    const path_t& dir,
    const find_include_predicate_t& include_predicate,
    const find_descend_predicate_t& descend_predicate,
    size_t depth
) {
    std::vector<rooted_path_t> result;

    std::error_code ec;
    for (auto& entry : std::filesystem::directory_iterator(dir.to_native_path(), ec)) {
        const auto path = path_t(dir.to_native_path() / entry.path().filename());

        if (include_predicate(path)) {
            result.push_back(rooted_path_t(root, root.relative(path)));
        }

        const bool is_symlink = entry.is_symlink(ec);
        if (ec) {
            break ;
        }

        if (!is_symlink && entry.is_directory(ec)) {
            if (ec) {
                break ;
            }

            if (descend_predicate(path, depth)) {
                auto subresult = find_impl(root, path, include_predicate, descend_predicate, depth + 1);
                result.insert(result.end(), std::make_move_iterator(subresult.begin()), std::make_move_iterator(subresult.end()));
            }
        }
    }
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::find: failed to iterate directory '{}': {}", dir, ec.message()));
    }

    return result;
}

std::vector<rooted_path_t> find(const path_t& root, const find_include_predicate_t& include_predicate, const find_descend_predicate_t& descend_predicate) {
    return find_impl(root, root, include_predicate, descend_predicate, 0);
}

path_t canonical(const path_t& path) {
    // std::cout << std::format("realpath {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    const auto result_path = std::filesystem::canonical(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::canonical: failed to get canonical path of '{}': {}", path, ec.message()));
    }

    return path_t(result_path);
}

void copy(const path_t& src, const path_t& dst) {
    // std::cout << std::format("cp -r {} {}", pretty_path_t(src), pretty_path_t(dst)) << std::endl;

    const auto parent = dst.parent();
    if (!exists(parent)) {
        create_directories(parent);
    }

    std::error_code ec;
    std::filesystem::copy(src.to_native_path(), dst.to_native_path(), std::filesystem::copy_options::recursive, ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::copy: failed to copy from '{}' to '{}': {}", src, dst, ec.message()));
    }
}

void touch(const path_t& path) {
    // std::cout << std::format("touch {}", pretty_path_t(path)) << std::endl;

    const auto parent = path.parent();
    if (!exists(parent)) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::touch: parent directory '{}' does not exist for path '{}'", parent, path));
    }

    if (exists(path)) {
        std::error_code ec;
        std::filesystem::last_write_time(path.to_native_path(), std::filesystem::file_time_type::clock::now(), ec);
        if (ec) {
            throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::touch: failed to update last write time of path '{}': {}", path, ec.message()));
        }
        return ;
    }

    std::ofstream ofs(path.to_native_path());
    if (!ofs) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::touch: failed to create file '{}'", path));
    }
}

void create_directories(const path_t& path) {
    // std::cout << std::format("mkdir -p {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    std::filesystem::create_directories(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::create_directories: failed to create directories for path '{}': {}", path, ec.message()));
    }
}

void create_symlink(const path_t& src, const path_t& dst) {
    // std::cout << std::format("ln -s {} {}", pretty_path_t(src), pretty_path_t(dst)) << std::endl;

    std::error_code ec;
    std::filesystem::create_symlink(src.to_native_path(), dst.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::create_symlink: failed to create symlink from '{}' to '{}': {}", dst, src, ec.message()));
    }
}

void create_directory_symlink(const path_t& src, const path_t& dst) {
    // std::cout << std::format("ln -s {} {}", pretty_path_t(src), pretty_path_t(dst)) << std::endl;

    if (m03gagbhsnusi43zogoacgj2ez_filesystem::exists(dst)) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::create_directory_symlink: destination path '{}' already exists", dst));
    }

    std::error_code ec;
    std::filesystem::create_directory_symlink(src.to_native_path(), dst.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::create_directory_symlink: failed to create directory symlink from '{}' to '{}': {}", dst, src, ec.message()));
    }
}

path_t current_path() {
    // std::cout << "pwd" << std::endl;

    std::error_code ec;
    const auto result_path = std::filesystem::current_path(ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::current_path: failed to get current working directory: {}", ec.message()));
    }

    return path_t(result_path);
}

void current_path(const path_t& path) {
    // std::cout << std::format("cd {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    std::filesystem::current_path(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::current_path: failed to set current working directory to '{}': {}", path, ec.message()));
    }
}

bool exists(const path_t& path) {
    // std::cout << std::format("test -e {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    const bool result = std::filesystem::exists(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::exists: failed to check existence of path '{}': {}", path, ec.message()));
    }
    return result;
}

std::uintmax_t file_size(const path_t& path) {
    // std::cout << std::format("stat -c%s {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    const std::uintmax_t result = std::filesystem::file_size(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::file_size: failed to get file size of path '{}': {}", path, ec.message()));
    }
    return result;
}

std::filesystem::file_time_type last_write_time(const path_t& path) {
    // std::cout << std::format("stat -c%Y {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    std::filesystem::file_time_type result = std::filesystem::last_write_time(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::last_write_time: failed to get last write time of path '{}': {}", path, ec.message()));
    }
    return result;
}

bool remove(const path_t& path) {
    // std::cout << std::format("rm {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    const bool result = std::filesystem::remove(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::remove: failed to remove path '{}': {}", path, ec.message()));
    }
    return result;
}

std::uintmax_t remove_all(const path_t& path) {
    // std::cout << std::format("rm -rf {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    const std::uintmax_t result = std::filesystem::remove_all(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::remove_all: failed to remove all at path '{}': {}", path, ec.message()));
    }
    return result;
}

void rename_strict(const path_t& from, const path_t& to) {
    // std::cout << std::format("mv {} {}", pretty_path_t(from), pretty_path_t(to)) << std::endl;

    if (exists(to)) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::rename_strict: destination path '{}' already exists", to));
    }

    std::error_code ec;
    std::filesystem::rename(from.to_native_path(), to.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::rename_strict: failed to rename path '{}' to '{}': {}", from, to, ec.message()));
    }
}

void rename_replace(const path_t& from, const path_t& to) {
    // std::cout << std::format("mv -f {} {}", pretty_path_t(from), pretty_path_t(to)) << std::endl;

    std::error_code ec;
    std::filesystem::rename(from.to_native_path(), to.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::rename_replace: failed to rename path '{}' to '{}': {}", from, to, ec.message()));
    }
}

bool is_regular_file(const path_t& path) {
    // std::cout << std::format("test -f {}", pretty_path_t(path)) << std::endl;

    std::error_code ec;
    const bool result = std::filesystem::is_regular_file(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::is_regular_file: failed to check if path '{}' is a regular file: {}", path, ec.message()));
    }
    return result;
}

bool is_directory(const path_t& path) {
    // std::cout << std::format("test -d {}", pretty_path_t(path)) << std::endl;
    
    std::error_code ec;
    const bool result = std::filesystem::is_directory(path.to_native_path(), ec);
    if (ec) {
        throw std::runtime_error(std::format("m03gagbhsnusi43zogoacgj2ez_filesystem::is_directory: failed to check if path '{}' is a directory: {}", path, ec.message()));
    }
    return result;
}

} // namespace m03gagbhsnusi43zogoacgj2ez_filesystem
