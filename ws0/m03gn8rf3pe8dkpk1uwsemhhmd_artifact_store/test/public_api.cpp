...
int main() {
    return test::run([] {
        ...
        test::expect(!api::exists_or_symlink(missing));
        ...
        test::expect(api::exists_or_symlink(regular_file));
        ...
        test::expect(api::exists_or_symlink(directory));
        ...
        test::expect(api::exists_or_symlink(symlink));
        api::remove_existing_path(symlink);
        test::expect(!api::exists_or_symlink(symlink));
        test::expect(std::filesystem::exists(symlink_target.to_native_path()));
        ...
        test::expect(found.has_value());
        test::expect_equal(*found, newer);
        test::expect(!api::completed_artifact_dir_by_hash(lookup_root, "missing"));
        test::expect_equal(api::versioned_artifact_dir(lookup_root, "abc"), newer);

        const auto generated = api::versioned_artifact_dir(lookup_root, "newhash");
        test::expect_equal(generated.parent(), lookup_root);
        test::expect(std::regex_match(
            generated.filename(),
            std::regex(R"([0-9]{8}T[0-9]{6}\.[0-9]{9}Z-newhash)")
        ));
        test::expect(!api::exists_or_symlink(generated));
        ...
        test::expect_equal(module_artifact.parent(), expected_kind_root);
        test::expect(module_artifact.filename().ends_with("-modulehash"));
        test::expect_equal(
            api::latest_dir(module, kind),
        ...
        test::expect_equal(
            api::artifact_dir(module, kind, "modulehash"),
        ...
        test::expect(std::filesystem::is_symlink(latest_path.to_native_path()));
        test::expect_equal(resolved_symlink_target(latest_path), target_a);
        test::expect(!api::exists_or_symlink(latest_path + "_tmp"));

        api::update_latest_symlink(latest_path, target_a);
        test::expect_equal(resolved_symlink_target(latest_path), target_a);
        api::update_latest_symlink(latest_path, target_b);
        test::expect_equal(resolved_symlink_target(latest_path), target_b);
        ...
        test::expect_equal(resolved_symlink_target(preexisting_tmp_latest), target_a);
        test::expect(!api::exists_or_symlink(preexisting_tmp));
        ...
        test::expect(std::filesystem::is_directory(parent_symlink.to_native_path()));
        test::expect(!std::filesystem::is_symlink(parent_symlink.to_native_path()));
        test::expect_equal(resolved_symlink_target(through_parent_symlink), target_b);
        ...
        test::expect(std::filesystem::is_symlink(module_latest.to_native_path()));
        test::expect_equal(resolved_symlink_target(module_latest), module_target);
        api::update_latest(module, kind, target_a);
        test::expect_equal(resolved_symlink_target(module_latest), target_a);
    });
}
