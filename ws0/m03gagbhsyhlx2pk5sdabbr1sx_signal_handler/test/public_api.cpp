...
void read_ready(int fd) {
    ...
    test::expect_equal(count, ssize_t(1));
    test::expect_equal(value, 'r');
}
...
int main() {
    return test::run([] {
        ...
        test::expect_equal(request.signal_number(), SIGTERM);
        test::expect_equal(
            std::string(request.what()),
            std::string("termination requested by signal ") + std::to_string(SIGTERM)
        );

        test::expect_equal(run_in_subprocess([] {
...

        {
            ...
            test::expect_equal(guard.pid(), pid_t(-1));
...

        {
            ...
            pid_t child_pid = -1;
            {
                ...
                child_pid = guard.pid();
                test::expect(0 < child_pid);
                test::expect_equal(waitpid(child_pid, &status, 0), child_pid);
            }
...

        {
            ...
            test::expect(WIFEXITED(status));
            test::expect_equal(WEXITSTATUS(status), 127);
        }

        {
            default_signal_action_t default_sigterm(SIGTERM);
            int ready_pipe[2];
            test::expect_equal(pipe(ready_pipe), 0);
            ...
            try {
                {
                    ...
                    test::expect_equal(kill(getpid(), SIGTERM), 0);
                    test::expect_equal(waitpid(child_pid, &status, 0), child_pid);
                }
            } catch (const api::termination_request_t& termination) {
                caught = true;
                test::expect_equal(termination.signal_number(), SIGTERM);
            }

            test::expect(caught);
            test::expect(WIFSIGNALED(status));
            test::expect_equal(WTERMSIG(status), SIGTERM);
        }
            ...
            try {
                {
                    ...
                    test::expect_equal(guard.pid(), child_pid);
                    read_ready(ready_pipe[0]);
                    close(ready_pipe[0]);

                    test::expect_equal(kill(getpid(), SIGHUP), 0);
                    test::expect_equal(waitpid(child_pid, &status, 0), child_pid);
                }
            } catch (const api::termination_request_t& termination) {
                caught = true;
                test::expect_equal(termination.signal_number(), SIGHUP);
            }

            test::expect(caught);
            test::expect(WIFSIGNALED(status));
            test::expect_equal(WTERMSIG(status), SIGHUP);
        }
    });
}
