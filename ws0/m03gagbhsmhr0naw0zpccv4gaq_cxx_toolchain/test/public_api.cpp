...
int main() {
    return test::run([] {
        ...
        test::expect_equal(define.key(), std::string("_VALID_9"));
        test::expect_equal(define.value(), std::string("value\"with\\escapes"));
        ...
        test::expect(empty_link_inputs.libraries.empty());
        ...
        test::expect_equal(built_library, library_output);
        test::expect(filesystem_api::is_regular_file(built_library));
        ...
        test::expect(handle != nullptr, dlerror() == nullptr ? "dlopen failed" : dlerror());
        ...
        test::expect(combined_value != nullptr);
        test::expect(configured_message != nullptr);
        test::expect_equal(combined_value(), 42);
        test::expect_equal(std::string(configured_message()), message);
        test::expect_equal(dlclose(handle), 0);

        api::link_inputs_t link_inputs {
            .libraries = { built_library }
        };
        test::expect_equal(link_inputs.libraries.size(), std::size_t(1));
        test::expect_equal(link_inputs.libraries[0], built_library);
        ...
        test::expect_equal(built_binary, binary_output);
        test::expect(filesystem_api::is_regular_file(built_binary));
        test::expect(filesystem_api::is_regular_file(
            binary_build / filesystem_api::relative_path_t("app/main.o")
        ));
        test::expect_equal(run_binary(built_binary), 0);
