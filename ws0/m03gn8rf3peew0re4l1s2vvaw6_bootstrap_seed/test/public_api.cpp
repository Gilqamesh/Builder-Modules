...
int main() {
    return test::run([] {
        ...
        test::expect_equal(
            seed_module.name().unique_name(),
            std::string("m03gagbhst621faiop1rztfkqp_builder_cli")
        );
        test::expect_equal(&seed_module, &api::module(graph));
        test::expect(api::is_module(seed_module));

        const auto modules = api::modules(graph);
        test::expect_equal(modules.size(), bootstrap_module_names.size());
        for (std::size_t i = 0; i < modules.size(); ++i) {
            test::expect_equal(modules[i]->name().unique_name(), bootstrap_module_names[i]);
            test::expect(api::is_module(*modules[i]));
            modules[i]->version(graph_api::version_t(static_cast<std::uint64_t>(i + 1)));
        }
        modules[5]->version(graph_api::version_t(900));
        test::expect_equal(api::version(graph).value, std::uint64_t(900));
        ...
        test::expect(!api::is_module(unknown_module));
        ...
        test::expect(!api::is_module(misplaced_known_module));
        ...
        test::expect_equal(api::builder_plugin_path(graph), expected_plugin);
