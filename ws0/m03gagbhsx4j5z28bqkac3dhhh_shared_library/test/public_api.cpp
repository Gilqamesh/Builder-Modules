...
int main() {
    return test::run([] {
...

        {
            ...
            test::expect_equal(std::system(command.c_str()), 0);
        }
...

        {
            ...
            test::expect_equal(answer(), 11);

            const auto optional_answer = loader.resolve_optional("local_answer");
            test::expect(optional_answer.has_value());
            nullary_fn_t optional_fn = *optional_answer;
            test::expect_equal(optional_fn(), 11);

            test::expect(!loader.resolve_optional("missing_symbol"));
            test::expect_throws<std::runtime_error>([&] {
                [[maybe_unused]] const auto missing = loader.resolve("missing_symbol");
            });

            dlerror();
            test::expect_equal(dlsym(RTLD_DEFAULT, "local_answer"), nullptr);
        }

        {
            ...
            test::expect_equal(answer(), 22);

            dlerror();
            auto* global_symbol = dlsym(RTLD_DEFAULT, "global_answer");
            test::expect(global_symbol != nullptr);
            auto global_fn = reinterpret_cast<nullary_fn_t>(global_symbol);
            test::expect_equal(global_fn(), 22);
        }

        test::expect(!is_loaded(dtor_library));
        {
            api::loader_t loader(
                dtor_library,
                api::lifetime_t::DTOR,
            ...
            test::expect_equal(answer(), 33);
        }
        test::expect(!is_loaded(dtor_library));

        test::expect(!is_loaded(process_library));
        {
            api::loader_t loader(
                process_library,
...

        {
            ...
            test::expect_equal(add(19, 23), 42);
            ...
            test::expect_equal(&(destination = std::move(moved)), &destination);
            add = destination.resolve("add_values");
            test::expect_equal(add(-3, 8), 5);
            test::expect_equal(&(destination = std::move(destination)), &destination);
            add = destination.resolve("add_values");
            test::expect_equal(add(1, 2), 3);
        }
