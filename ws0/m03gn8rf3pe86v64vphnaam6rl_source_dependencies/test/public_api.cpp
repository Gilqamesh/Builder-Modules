...
int main() {
    return test::run([] {
        ...
        test::expect(empty_scan.local_files.empty());
        test::expect(empty_scan.dependencies.empty());
        ...
        test::expect_equal(
            relative_strings(scan.local_files),
            std::vector<std::string>({ "local.h", "main.cpp", "nested/nested.hpp" })
        );
        test::expect_equal(scan.dependencies.size(), std::size_t(2));
        test::expect_equal(
            pointer_set(scan.dependencies),
            std::set<const graph_api::module_t*>({ dependency_a, dependency_b })
        );
        ...
        test::expect_equal(excluded_scan.dependencies.size(), std::size_t(1));
        test::expect_equal(excluded_scan.dependencies[0], dependency_a);
        test::expect_equal(relative_strings(excluded_scan.local_files), relative_strings(scan.local_files));
        ...
        test::expect_equal(
            pointer_set(builder_scan.dependencies),
            std::set<const graph_api::module_t*>({ dependency_a, dependency_b })
        );
        ...
        test::expect_equal(same_workspace_scan.dependencies.size(), std::size_t(1));
        test::expect_equal(same_workspace_scan.dependencies[0], same_workspace);
        ...
        test::expect_equal(dependencies_without_owner.size(), std::size_t(3));
        test::expect_equal(
            pointer_set(dependencies_without_owner),
            std::set<const graph_api::module_t*>({ dependency_a, dependency_b, dependency_c })
        );
        ...
        test::expect_equal(dependencies_with_owner.size(), std::size_t(4));
        test::expect_equal(dependencies_with_owner.front(), owner);
        test::expect_equal(
            pointer_set(dependencies_with_owner),
            std::set<const graph_api::module_t*>({ owner, dependency_a, dependency_b, dependency_c })
        );
        ...
        test::expect_equal(
            pointer_set(builder_dependencies),
            std::set<const graph_api::module_t*>({ dependency_a, dependency_b, dependency_c })
        );

        const auto dependency_names = api::module_names(dependencies_with_owner);
        test::expect_equal(dependency_names.size(), dependencies_with_owner.size());
        for (std::size_t i = 0; i < dependency_names.size(); ++i) {
            test::expect_equal(dependency_names[i], dependencies_with_owner[i]->name());
        }
        test::expect(api::module_names({}).empty());
    });
}
