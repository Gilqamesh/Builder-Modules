...
int main() {
    return test::run([] {
        ...
        test::expect(empty_dependencies.module_dependencies.empty());
        test::expect(empty_dependencies.builder_dependencies.empty());
        ...
        test::expect(subject != nullptr);
        test::expect(discovery_owner != nullptr);
        ...
        test::expect_equal(source_installed.root(), arbitrary_root);
        test::expect_equal(interface_installed.root(), arbitrary_root);
        test::expect_equal(library_installed.root(), arbitrary_root);

        {
            api::source_phase_t source_phase(*subject, nullptr);
            test::expect_equal(source_phase.name(), std::string_view("source"));
            test::expect_equal(source_phase.source_dir(), subject->source_dir());
            ...
            test::expect(filesystem_api::is_directory(source_phase.build_dir()));
            ...
            test::expect_equal(built_external.rooted_path().root(), source_phase.build_dir());
            test::expect_equal(
                built_external.rooted_path().relative_path(),
                filesystem_api::relative_path_t("external.alias")
            );
            source_phase.install(built_external);
            test::expect(filesystem_api::exists(
...

        {
            ...
            test::expect_equal(interface_phase.name(), std::string_view("interface"));
            ...
            test::expect_equal(built_path.rooted_path().root(), source_output.root());
            test::expect_equal(
                built_path.rooted_path().relative_path(),
                filesystem_api::relative_path_t("data.txt")
            );
            ...
            test::expect_equal(built_rooted.rooted_path().path(), manual_rooted.path());
            ...
            test::expect_equal(
                selected_source.rooted_path().relative_path(),
            ...
            test::expect(filesystem_api::is_regular_file(staged));
...

        {
            ...
            test::expect_equal(library_phase.name(), std::string_view("library"));
            test::expect(library_phase.validations().empty());
            ...
            test::expect_equal(library_phase.validations().size(), std::size_t(1));
            const auto& validation = library_phase.validations().front();
            test::expect_equal(validation.name, std::string("validation"));
            test::expect_equal(validation.source_files.size(), std::size_t(1));
            test::expect_equal(
                validation.source_files[0].rooted_path().relative_path(),
                filesystem_api::relative_path_t("api.cpp")
            );
            test::expect_equal(validation.defines.size(), std::size_t(1));
            test::expect_equal(validation.defines[0].key(), std::string("VALIDATION_DEFINE"));
            test::expect_equal(
            ...
            test::expect(filesystem_api::is_regular_file(library_path));
            test::expect_equal(library_phase.install<api::library_phase_t>().root(), installed.root());
            ...
            test::expect_equal(aggregate_validation.name, std::string("aggregate"));
            test::expect_equal(aggregate_validation.source_files.size(), std::size_t(1));
            test::expect_equal(aggregate_validation.arguments.size(), std::size_t(1));
        }

        {
            ...
            test::expect_equal(installed.root(), fake_binary_root);
            test::expect_equal(installed.target("tool"), fake_binary);
            test::expect_throws<std::runtime_error>([&] {
...

        {
            ...
            test::expect_equal(binary_phase.name(), std::string_view("binary"));
            test::expect(binary_phase.should_install_target("cli"));
            test::expect(binary_phase.should_install_target("anything"));

            const auto installed = binary_phase.install<api::binary_phase_t>();
            const auto cli = installed.target("cli");
            test::expect_equal(run_binary(cli), 0);
            ...
            test::expect_equal(run_binary(extra), 0);
            binary_phase.install_binary("extra", { cli_source });
            test::expect_equal(run_binary(installed.target("extra")), 0);
...

        {
            ...
            test::expect(selected.should_install_target("selected"));
            test::expect(!selected.should_install_target("other"));
...

        {
            ...
            test::expect_equal(phase->name(), std::string_view("binary"));
            auto* binary = dynamic_cast<api::binary_phase_t*>(phase.get());
            test::expect(binary != nullptr);
            test::expect(binary->should_install_target("any"));

            auto selected_phase = api::phase_base_t::make(*subject, "chosen");
            auto* selected = dynamic_cast<api::binary_phase_t*>(selected_phase.get());
            test::expect(selected != nullptr);
            test::expect(selected->should_install_target("chosen"));
            test::expect(!selected->should_install_target("other"));
            test::expect_equal(selected_phase->install<api::source_phase_t>().root(),
                phase->install<api::source_phase_t>().root());
        }

        const auto dependencies = api::discover_module_dependencies(*discovery_owner);
        test::expect_equal(dependencies.module_dependencies.size(), std::size_t(1));
        test::expect_equal(dependencies.builder_dependencies.size(), std::size_t(1));
        test::expect(contains_name(dependencies.module_dependencies, module_dependency_name));
        test::expect(contains_name(dependencies.builder_dependencies, builder_dependency_name));
    });
}
