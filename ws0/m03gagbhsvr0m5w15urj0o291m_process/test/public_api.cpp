...
int main() {
    return test::run([] {
        ...
        test::expect_equal(variable.name(), std::string("NAME"));
        test::expect_equal(variable.value(), std::string("value with spaces"));
        test::expect_equal(
            std::format("{}", variable),
        ...
        test::expect(empty_command.args().empty());
        test::expect(!empty_command.working_dir());
        test::expect(empty_command.environment_variables().empty());
        test::expect_equal(
            std::format("{}", empty_command),
        ...
        test::expect_equal(
            described_command.args(),
            std::vector<std::string>({ "/bin/echo", "hello world" })
        );
        test::expect(described_command.working_dir().has_value());
        test::expect_equal(*described_command.working_dir(), working_dir);
        test::expect_equal(described_command.environment_variables().size(), std::size_t(2));
        test::expect_equal(described_command.environment_variables()[0].name(), std::string("FIRST"));
        test::expect_equal(described_command.environment_variables()[1].value(), std::string("two"));
        test::expect_equal(
            std::format("{}", described_command),
            std::format(
        ...
        test::expect_equal(api::create_and_wait(api::command_t({ "/bin/true" })), 0);
        test::expect_equal(
            api::create_and_wait(api::command_t({ "/bin/sh", "-c", "exit 7" })),
            7
        );
        test::expect_equal(
            api::create_and_wait(api::command_t({ "/bin/sh", "-c", "kill -TERM $$" })),
            -SIGTERM
        );
        test::expect_equal(
            api::create_and_wait(api::command_t({ "/definitely/not/an/executable" })),
            127
        );
        test::expect_equal(api::create_and_wait(empty_command), 127);
...

        {
            ...
            test::expect_equal(WEXITSTATUS(status), 23);
        }
