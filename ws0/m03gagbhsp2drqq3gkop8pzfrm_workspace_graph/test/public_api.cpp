...
int main() {
    return test::run([] {
        test::expect_equal(std::string(api::BUILDER_CPP), std::string("builder.cpp"));
        test::expect_equal(std::string(api::CLI_CPP), std::string("cli.cpp"));

        const api::version_t direct_version(42);
        test::expect_equal(direct_version.value, std::uint64_t(42));
        ...
        test::expect_equal(api::version_t(file_time).value, expected_file_time_version);
        ...
        test::expect_equal(known_name.unique_name(), known_unique_name);
        test::expect_equal(known_name.friendly_name(), std::string("byte_stream"));
        test::expect_equal(known_name.uuid().version(), 7U);
        test::expect_equal(known_name.uuid().bytes().size(), std::size_t(16));
        test::expect_equal(std::format("{}", known_name), known_unique_name);
        test::expect_equal(
            api::module_name_hash_t()(known_name),
        ...
        test::expect_equal(generated_name.friendly_name(), std::string("Friendly_7"));
        test::expect_equal(generated_name.unique_name().size(), std::size_t(37));
        test::expect_equal(generated_name.unique_name()[0], 'm');
        test::expect_equal(generated_name.unique_name()[26], '_');
        test::expect_equal(generated_name.uuid().version(), 7U);
        test::expect_equal(api::module_name_t(generated_name.unique_name()), generated_name);

        const api::module_name_t later_name("m03gagbhtft23yhjwpp881tfmc_uuid");
        test::expect(known_name == api::module_name_t(known_unique_name));
        test::expect((known_name < later_name) || (later_name < known_name));
        test::expect(known_name <= known_name);
        ...
        test::expect_equal(ws0.relative_path(), filesystem_api::relative_path_t("ws0"));
        test::expect_equal(ws0.order_position(), std::uint32_t(0));
        test::expect_equal(ws10.order_position(), std::uint32_t(10));
        test::expect(ws0 == api::workspace_name_t("ws0"));
        test::expect(ws0 < ws2);
        test::expect(ws2 < ws10);
        test::expect(ws0 <= ws0);
        test::expect(ws0 <= ws10);
        test::expect_equal(std::format("{}", ws10), std::string("ws10"));
        test::expect_equal(
            api::workspace_name_hash_t()(ws2),
        ...
        test::expect_equal(
            api::version_t(version_directory).value,
        ...
        test::expect_equal(graph.root(), root);
        test::expect_equal(graph.artifact_root(), artifacts);
        test::expect(graph.modules().empty());

        const auto workspaces = graph.workspaces();
        test::expect_equal(workspaces.size(), std::size_t(2));
        test::expect_equal(workspaces[0]->name(), ws0);
        test::expect_equal(workspaces[1]->name(), ws2);
        test::expect_equal(&workspaces[0]->graph(), &graph);
        test::expect_equal(std::format("{}", *workspaces[0]), std::string("ws0"));

        const auto indexed_names = graph.module_names();
        test::expect_equal(indexed_names.size(), std::size_t(3));
        test::expect(indexed_names.contains(known_name));
        test::expect(indexed_names.contains(later_name));
        test::expect(indexed_names.contains(third_name));

        auto* first_module = graph.discover_module(known_name);
        test::expect_equal(first_module, graph.discover_module(known_name));
        test::expect_equal(&first_module->workspace(), workspaces[0]);
        test::expect_equal(first_module->name(), known_name);
        test::expect_equal(first_module->source_dir(), filesystem_api::path_t(root_native / "ws0" / known_name.unique_name()));
        test::expect_equal(first_module->artifact_base_dir(), filesystem_api::path_t(artifacts_native / known_name.unique_name()));
        test::expect_equal(first_module->artifact_latest_dir(), filesystem_api::path_t(artifacts_native / known_name.unique_name() / "latest"));
        test::expect_equal(std::format("{}", *first_module), known_name.unique_name());
        test::expect_equal(workspaces[0]->find_module(known_name), first_module);
        test::expect_equal(workspaces[0]->find_module(third_name), nullptr);

        const auto discovered_version = first_module->version();
        test::expect(0 < discovered_version.value);
        first_module->version(api::version_t(123));
        test::expect_equal(first_module->version().value, std::uint64_t(123));

        auto* third_module = graph.discover_module(third_name);
        test::expect_equal(&third_module->workspace(), workspaces[1]);
        const auto discovered_modules = graph.modules();
        test::expect_equal(discovered_modules.size(), std::size_t(2));
        test::expect_equal(discovered_modules[0], first_module);
        test::expect_equal(discovered_modules[1], third_module);
        ...
        test::expect(manual_workspace == api::workspace_t(graph, api::workspace_name_t("ws9")));
        test::expect(manual_workspace < later_workspace);
        test::expect(manual_workspace <= manual_workspace);
        test::expect_equal(&manual_workspace.graph(), &graph);
        test::expect_equal(manual_workspace.name(), api::workspace_name_t("ws9"));
        test::expect_equal(std::format("{}", manual_workspace), std::string("ws9"));
        ...
        test::expect_equal(manual_workspace.find_module(manual_name_a), manual_module_a);
        test::expect_equal(manual_workspace.find_module(manual_name_b), manual_module_b);
        test::expect_equal(manual_workspace.find_module(known_name), nullptr);

        const auto manual_modules = manual_workspace.modules();
        test::expect_equal(manual_modules.size(), std::size_t(2));
        test::expect(manual_modules[0]->name() < manual_modules[1]->name());
        test::expect_equal(&manual_module_a->workspace(), &manual_workspace);
        test::expect_equal(manual_module_a->name(), manual_name_a);
        test::expect_equal(manual_module_a->version().value, std::uint64_t(1));
        manual_module_a->version(api::version_t(9));
        test::expect_equal(manual_module_a->version().value, std::uint64_t(9));
        test::expect_equal(
            manual_module_a->source_dir(),
            root / api::workspace_name_t("ws9").relative_path() / filesystem_api::relative_path_t(manual_name_a.unique_name())
        );
        test::expect_equal(
            manual_module_a->artifact_base_dir(),
            artifacts / filesystem_api::relative_path_t(manual_name_a.unique_name())
        );
        test::expect_equal(
            manual_module_a->artifact_latest_dir(),
        ...
        test::expect_equal(explicit_context.workspace_root, root);
        test::expect_equal(explicit_context.artifact_root, filesystem_api::path_t(explicit_artifacts_native));
        test::expect_equal(std::string(std::getenv("BUILDER_WORKSPACE_ROOT")), root.string());
        test::expect_equal(
            std::string(std::getenv("BUILDER_ARTIFACT_ROOT")),
        ...
        test::expect_equal(default_context.workspace_root, root);
        test::expect_equal(default_context.artifact_root, root / filesystem_api::relative_path_t("artifacts"));
        ...
        test::expect_equal(aggregate_context.workspace_root, root);
        test::expect_equal(aggregate_context.artifact_root, artifacts);
    });
}
